...
def main():
    ...
    outdf = {'session': [], 'temperature': [], 'genotype': [], 'substrate': [], 'total_duration': []}
    ...
    if os.path.isfile(hook_file) and not OVERWRITE:
        ...
        for i_ses, each in enumerate(sorted(sessions)):
            ...
            try:
                ...
                csv_file2 = os.path.join(in2folder, '{}_{}.csv'.format(each[:-4], _in2+'_etho'))
    ...
    for j, sub in enumerate(['yeast', 'sucrose']):
        ...
        ax = sns.boxplot(x='genotype', y='total_duration', hue='temperature', data=rdata, order=conds, hue_order=['22ºC', '30ºC'], palette=mypal, saturation=1.,width=.5, ax=ax, boxprops=dict(lw=0.0), showfliers=False)
        ax = sns.swarmplot(x='genotype', y='total_duration', hue='temperature', data=rdata, order=conds, hue_order=['22ºC', '30ºC'], dodge=True, color='#666666', size=2, ax=ax)
        ax.legend_.remove()
