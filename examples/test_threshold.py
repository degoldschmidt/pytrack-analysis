...
yeasts = [_file for _file in os.listdir(os.path.join('..','media','templates', setup)) if 'yeast' in _file]
arenas = [_file for _file in os.listdir(os.path.join('..','media','templates', setup)) if 'arena' in _file]
print(yeasts, arenas)
templates = [cv2.imread(os.path.join('..','media','templates', setup, _file),0) for _file in yeasts]
templates_arena = [cv2.imread(os.path.join('..','media','templates', setup, _file),0) for _file in arenas]
    ...
    if len(patches) == 0:
        ...
        for i, each_patch in enumerate(patches):
            for eachpt in each_patch:
                if abs(eachpt[0]-pt[0]) < w_arena and abs(eachpt[1]-pt[1]) < w_arena:
                    patches[i].append(pt)
                    break
